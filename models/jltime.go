...
func DailyDeductCronWrapper() {
	DailyDeduct(nil)
}

func DailyDeduct(sender *Sender) {
	if sender != nil {
		logs.Info("===== 用户【%d】触发每日扣费任务 =====", sender.UserID)
	} else {
		logs.Info("===== [定时任务] 开始执行每日扣费 =====")
	}

	var projects []ActivityProject
	err := db.Where("is_daily_deduct = ? AND status = 0 AND expire_date != '' AND deleted_at IS NULL", true).
		Find(&projects).Error
	if err != nil {
		logs.Error("查询按天计费项目失败：%v", err)
		return
	}

	if len(projects) == 0 {
		logs.Info("无按天计费项目需要扣费")
		return
	}
	...
	for _, project := range projects {
		...
		if now.After(expireThreshold) || now.Equal(expireThreshold) {
			...
			project.SyncError = ""
			if err := UpdateActivityProject(&project); err != nil {
				logs.Error("禁用到期项目失败 ID=%d: %v", project.ID, err)
				continue
			}
			go TriggerSync(project.ID)
			disabled++
		...
		userCoin := GetCoin(project.UserNumber)
		if userCoin < dailyCoin {
			project.Status = 1
			project.SyncStatus = "pending_disable"
			project.SyncError = ""
			if err := UpdateActivityProject(&project); err != nil {
				logs.Error("禁用积分不足项目失败 ID=%d: %v", project.ID, err)
				continue
			}
			go TriggerSync(project.ID)
			disabled++
			...
			PushByQQ(strconv.Itoa(project.UserNumber), fmt.Sprintf(
				"⚠️【按天扣费失败通知】\n活动：%s\n账号：%s\n原因：积分不足（当前%d，需要%d）\n已暂停授权，请及时充值积分。",
				cfg.Name, accountAlias, userCoin, dailyCoin))
			continue
		}

		RemCoin(project.UserNumber, dailyCoin)
		deducted++
		logs.Info("每日扣费成功：用户%d，活动%s，扣除%d积分", project.UserNumber, cfg.Name, dailyCoin)
	}

	logs.Info("每日扣费完成：扣费%d个，禁用%d个", deducted, disabled)
	if sender != nil {
		sender.Reply(fmt.Sprintf("每日扣费完成：扣费%d个，禁用%d个", deducted, disabled))
	}
}
