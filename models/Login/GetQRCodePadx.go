...
// 获取二维码(安卓Pad-绕过验证码) AndroidPadVersionx
func GetQRCODEPadx(Data GetQRReq) models.ResponseResult2 {
	...
	reqDataLogin := DataLogin{
		...
		Proxy:         Data.Proxy,
		ClientVersion: int32(Algorithm.AndroidPadVersionx),
	}
	if D == nil || D.Wxid == "" || D.ClientVersion != Algorithm.AndroidPadVersionx {
		// 没有缓存, 初始化新的账号环境
		D = GenAndroidPadLoginData(reqDataLogin)
	} else {
		D = UpdateAndroidPadLoginData(D, reqDataLogin)
	}
	...
	hec := InitHec(D)
	hypack := hec.HybridEcdhPackAndroidEn(502, 10002, 0, nil, reqdata)
