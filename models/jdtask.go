...
func CX_jd_fruit_new_cx() {
	cmd := exec.Command("python3", "scripts/jd_newfruit_cx.py")
	output, err := cmd.CombinedOutput()
	if err != nil {
	...
	for _, line := range lines {
		if line != "" {
			...
			if len(parts) >= 2 {
				msgs := []string{
					fmt.Sprintf("【通知】东东农场新版："),
				}
				pin := parts[0]
				msg := parts[1]
				msgs = append(msgs, fmt.Sprintf("账号: %s", pin))
				msgs = append(msgs, msg)
				msgs = append(msgs, "请去京东APP新农场奖品右上角【记录】里兑换，不兑换会过期")
				encodedPin := url.QueryEscape(pin)
				for i := range cks {
					if cks[i].PtPin == encodedPin {
...
// 话费积分推送
func CX_jd_dwapp_cx() {
	cmd := exec.Command("python3", "scripts/jd_dwapp_cx.py")
	output, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Println("执行JavaScript脚本失败:", err)
		//return
	...
	for _, line := range lines {
		if line != "" {
			...
			if len(parts) >= 2 {
				msgs := []string{
					fmt.Sprintf("【通知】积分兑换话费："),
				}
				pin := parts[0]
				msg := parts[1]
				msgs = append(msgs, fmt.Sprintf("账号: %s", pin))
				msgs = append(msgs, msg)
				msgs = append(msgs, "此账号话费积分已经满足兑换10元话费，请去京东APP---我的---赚话费---去兑换，积分有有效期，如不兑换，过期损失极大")
				encodedPin := url.QueryEscape(pin)
				for i := range cks {
					if cks[i].PtPin == encodedPin {
						logs.Info(encodedPin)
...
// 保价成功推送
func CX_jd_OnceApply_cx() {
	cmd := exec.Command("python3", "scripts/baojia_cx.py")
	output, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Println("执行JavaScript脚本失败:", err)
		//return
	...
	for _, line := range lines {
		if line != "" {
			...
			if len(parts) >= 2 {
				msgs := []string{
					fmt.Sprintf("【通知】保价成功通知：\n查看记录可前往：京东APP—我的—客户服务—价格保护—申请记录"),
				}
				pin := parts[0]
				msg := parts[1]
				msgs = append(msgs, fmt.Sprintf("账号: %s", pin))
				msgs = append(msgs, msg)
				msgs = append(msgs, "您的帐号通过机器人保价成功.")
				encodedPin := url.QueryEscape(pin)
				for i := range cks {
					if cks[i].PtPin == encodedPin {
						logs.Info(encodedPin)
