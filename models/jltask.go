...
func HandleDeleteCK(sender *Sender) interface{} {
	...
	go func() {
		...
		confirm, exit := getUserInputByField(sender, msgChannel, InputField{
			Key:        "confirm_delete",
			Prompt:     fmt.Sprintf("确认要删除【%s】这个账号吗？（输入y确认，其他字符取消）", GetFirstRemarkParam(selectedRemarks)),
			Validator: func(s string) (bool, string) {
				if checkExit(s) {
					return false, "程序已退出"
				}
		...
		if strings.Contains(output, "删除成功") {
			sender.Reply(fmt.Sprintf("【%s】CK删除成功！", GetFirstRemarkParam(selectedRemarks)))
		} else {
