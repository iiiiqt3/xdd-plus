...
func RabbitSendSMS(ty string, phone string, sender *Sender) bool {
    ...
    for attempt := 1; attempt <= 5; attempt++ {
        sender.Reply(fmt.Sprintf("正在进行第%d次滑块验证...", attempt))
        time.Sleep(3 * time.Second)

        var autoReq *httplib.BeegoHTTPRequest
        path := "/bot/mck/AutoCaptcha"
        if ty == "wskey" {
            path = "/bot/wskey/AutoCaptcha"
        }
        autoReq = httplib.Post(fmt.Sprintf("%s%s?BotApiToken=%s", sysConfig.RabbitUrl, path, sysConfig.RabbitApiToken))
        ...
        if status != 666 && status != 505 {
            ...
            sender.Reply("验证失败，请重新登录。")
            return false
        }
    }

    // 超过最大尝试次数
    sender.Reply("滑块验证多次失败，请稍后重试。")
    return false
}
