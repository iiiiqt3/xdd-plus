...
func GetWxNickname(wxid string) string {
	...
	if nickname == "" {
		nickname, _ = jsonparser.GetString([]byte(response), "data")
	}
	return nickname
}
...
func SendWxGroupMsg(uid string, gid string, msg string) {
	...
	case "qx":
		...
		req.Header("User-Agent", random)
		marshal, _ := json.Marshal(reply)
		logs.Info("发送@消息请求体:", string(marshal))
		req.Body(string(marshal))
		s, _ := req.String()
		logs.Info("发送@消息响应:", s)
	default:
...
func SendWxImg2(uid string, msg string) {
	...
	case "qx":
		...
		response, err := req.String()
		if err != nil {
			logs.Error("发送图片消息失败:", err)
			return
		}
		logs.Info("发送图片消息响应:", response)
...
func TransferRequest(toWxid string, transferid string, money string) error {
	...
	resp, err := req.String()
	if err != nil {
		logs.Error("HTTP 请求失败:", err)
		return err
	}

	// 打印响应结果
	logs.Info("转账确认请求成功，响应结果:", resp)
	...
	if err != nil {
		logs.Error("解析响应失败:", err)
		return err
	}

	// 检查响应状态
	if !response.State {
		logs.Error("转账失败，原因:", response.Error)
...
func SendQQMsg(msg QQMessage) {
	...
	if msg.QQMsg.MessageType == "" {
		if msg.QQMsg.GroupID > 0 {
			...
			logs.Error("SendQQMsg: 无法确定消息类型，group_id和user_id均为空")
			return
		}
	}
