...
const activityAdminStatsCacheTTL = 30 * time.Minute
const activityAdminStatsRefreshDebounce = 5 * time.Minute
const activityAdminStatsQueryTimeout = 12
...
var activityAdminStatsCache = struct {
	sync.RWMutex
	stats       []ActivityStat
	totalAll    int
	validAll    int
...
var activityAuthListCache = struct {
	sync.RWMutex
	list        []ActivityAuthItem
	expiresAt   time.Time
	updatedAt   time.Time
	lastRefresh time.Time
	refreshing  bool
...
var activityStatsQLClients = struct {
	sync.Mutex
	clients map[string]*QingLongClient
}{clients: map[string]*QingLongClient{}}

func getActivityStatsQLClient(config *QingLongConfig) *QingLongClient {
	if config == nil {
		return nil
	}
	key := config.Name
	...
	activityStatsQLClients.Lock()
	defer activityStatsQLClients.Unlock()
	if client, ok := activityStatsQLClients.clients[key]; ok {
		return client
	}
	client := NewQingLongClientWithTimeout(config, activityAdminStatsQueryTimeout)
	activityStatsQLClients.clients[key] = client
	return client
}

func InvalidateActivityAdminStatsCache() {
	activityAdminStatsCache.Lock()
	activityAdminStatsCache.expiresAt = time.Time{}
	activityAdminStatsCache.stats = nil
	activityAdminStatsCache.Unlock()
	activityAuthListCache.Lock()
	activityAuthListCache.expiresAt = time.Time{}
	activityAuthListCache.list = nil
	activityAuthListCache.Unlock()
	activityStatsQLClients.Lock()
	activityStatsQLClients.clients = map[string]*QingLongClient{}
	activityStatsQLClients.Unlock()
}

func RefreshActivityAdminStatsCache(forceRefresh ...bool) {
	force := len(forceRefresh) > 0 && forceRefresh[0]
	if !markActivityStatsRefreshing(force) {
		return
	}
	defer unmarkActivityStatsRefreshing()
	stats, totalAll, validAll, expiringAll, expiredAll := buildActivityStats()
	activityAdminStatsCache.Lock()
	activityAdminStatsCache.stats = append([]ActivityStat(nil), stats...)
	activityAdminStatsCache.totalAll = totalAll
	activityAdminStatsCache.validAll = validAll
	activityAdminStatsCache.expiringAll = expiringAll
	activityAdminStatsCache.expiredAll = expiredAll
	activityAdminStatsCache.updatedAt = time.Now()
	activityAdminStatsCache.expiresAt = time.Now().Add(activityAdminStatsCacheTTL)
	activityAdminStatsCache.Unlock()

	if markActivityAuthRefreshing(force) {
		list := buildActivityAuthList()
		activityAuthListCache.Lock()
		activityAuthListCache.list = append([]ActivityAuthItem(nil), list...)
		activityAuthListCache.updatedAt = time.Now()
		activityAuthListCache.expiresAt = time.Now().Add(activityAdminStatsCacheTTL)
		activityAuthListCache.Unlock()
		unmarkActivityAuthRefreshing()
	}
}
...
func RefreshSingleActivityAdminStats(activityID string) error {
	...
	stat := buildActivityStat(*cfg)
	item := buildActivityAuthItem(*cfg)
	activityAdminStatsCache.Lock()
	stats := append([]ActivityStat(nil), activityAdminStatsCache.stats...)
	found := false
	for i := range stats {
		if stats[i].ActivityID == activityID {
			stats[i] = stat
			found = true
			break
		}
	...
	if !found {
		...
		sort.SliceStable(stats, func(i, j int) bool { return stats[i].DisplayOrder < stats[j].DisplayOrder })
	}
	activityAdminStatsCache.stats = stats
	activityAdminStatsCache.totalAll, activityAdminStatsCache.validAll, activityAdminStatsCache.expiringAll, activityAdminStatsCache.expiredAll = summarizeActivityStats(stats)
	activityAdminStatsCache.updatedAt = time.Now()
	activityAdminStatsCache.expiresAt = time.Now().Add(activityAdminStatsCacheTTL)
	activityAdminStatsCache.Unlock()

	activityAuthListCache.Lock()
	list := append([]ActivityAuthItem(nil), activityAuthListCache.list...)
	found = false
	for i := range list {
	...
	if !found {
		...
		sort.SliceStable(list, func(i, j int) bool { return list[i].DisplayOrder < list[j].DisplayOrder })
	}
	activityAuthListCache.list = list
	activityAuthListCache.updatedAt = time.Now()
	activityAuthListCache.expiresAt = time.Now().Add(activityAdminStatsCacheTTL)
	activityAuthListCache.Unlock()
	return nil
...
func markActivityStatsRefreshing(force bool) bool {
	activityAdminStatsCache.Lock()
	defer activityAdminStatsCache.Unlock()
	now := time.Now()
	if activityAdminStatsCache.refreshing {
		return false
	}
	if !force && !activityAdminStatsCache.lastRefresh.IsZero() && now.Sub(activityAdminStatsCache.lastRefresh) < activityAdminStatsRefreshDebounce {
		return false
	}
	activityAdminStatsCache.refreshing = true
	activityAdminStatsCache.lastRefresh = now
	return true
}

func unmarkActivityStatsRefreshing() {
	activityAdminStatsCache.Lock()
	activityAdminStatsCache.refreshing = false
	activityAdminStatsCache.Unlock()
}

func markActivityAuthRefreshing(force bool) bool {
	activityAuthListCache.Lock()
	defer activityAuthListCache.Unlock()
	now := time.Now()
	if activityAuthListCache.refreshing {
		return false
	}
	if !force && !activityAuthListCache.lastRefresh.IsZero() && now.Sub(activityAuthListCache.lastRefresh) < activityAdminStatsRefreshDebounce {
		return false
	}
	activityAuthListCache.refreshing = true
	activityAuthListCache.lastRefresh = now
	return true
}

func unmarkActivityAuthRefreshing() {
	activityAuthListCache.Lock()
	activityAuthListCache.refreshing = false
	activityAuthListCache.Unlock()
}

func getActivityStatsSnapshot() (ActivityStatsSnapshot, bool) {
	activityAdminStatsCache.RLock()
	defer activityAdminStatsCache.RUnlock()
	if activityAdminStatsCache.stats == nil {
		return ActivityStatsSnapshot{}, false
	}
	return ActivityStatsSnapshot{
		Stats:       append([]ActivityStat(nil), activityAdminStatsCache.stats...),
		TotalAll:    activityAdminStatsCache.totalAll,
		ValidAll:    activityAdminStatsCache.validAll,
		ExpiringAll: activityAdminStatsCache.expiringAll,
		ExpiredAll:  activityAdminStatsCache.expiredAll,
		Cached:      true,
		Refreshing:  activityAdminStatsCache.refreshing,
		UpdatedAt:   activityAdminStatsCache.updatedAt,
	}, true
}

func GetActivityStatsSnapshot(force bool) ActivityStatsSnapshot {
	if !force {
		if snapshot, ok := getActivityStatsSnapshot(); ok {
			if time.Since(snapshot.UpdatedAt) > activityAdminStatsCacheTTL && !snapshot.Refreshing {
				go RefreshActivityAdminStatsCache()
			}
			return snapshot
		}
		if markActivityStatsRefreshing(false) {
			go func() {
				defer unmarkActivityStatsRefreshing()
				stats, totalAll, validAll, expiringAll, expiredAll := buildActivityStats()
				activityAdminStatsCache.Lock()
		...
		return ActivityStatsSnapshot{Stats: []ActivityStat{}, Cached: false, Refreshing: true}
	}
	RefreshActivityAdminStatsCache(true)
	snapshot, _ := getActivityStatsSnapshot()
	return snapshot
}

func GetActivityStats() ([]ActivityStat, int, int, int, int) {
	snapshot := GetActivityStatsSnapshot(false)
	return snapshot.Stats, snapshot.TotalAll, snapshot.ValidAll, snapshot.ExpiringAll, snapshot.ExpiredAll
}
...
func GetActivityAuthListSnapshot(force bool) ([]ActivityAuthItem, bool, bool, time.Time) {
	if !force {
		activityAuthListCache.RLock()
		if activityAuthListCache.list != nil {
			list := append([]ActivityAuthItem(nil), activityAuthListCache.list...)
			refreshing := activityAuthListCache.refreshing
			updatedAt := activityAuthListCache.updatedAt
			activityAuthListCache.RUnlock()
			if time.Since(updatedAt) > activityAdminStatsCacheTTL && !refreshing {
				go RefreshActivityAdminStatsCache()
			}
			return list, true, refreshing, updatedAt
		}
		activityAuthListCache.RUnlock()
		if markActivityAuthRefreshing(false) {
			go func() {
				defer unmarkActivityAuthRefreshing()
				list := buildActivityAuthList()
				activityAuthListCache.Lock()
				activityAuthListCache.list = append([]ActivityAuthItem(nil), list...)
				activityAuthListCache.updatedAt = time.Now()
		...
		return []ActivityAuthItem{}, false, true, time.Time{}
	}
	RefreshActivityAdminStatsCache(true)
	activityAuthListCache.RLock()
	list := append([]ActivityAuthItem(nil), activityAuthListCache.list...)
	...
	updatedAt := activityAuthListCache.updatedAt
	activityAuthListCache.RUnlock()
	return list, len(list) > 0, refreshing, updatedAt
}
