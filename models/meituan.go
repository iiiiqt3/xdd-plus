...
func UpLine(token string, sender *Sender) bool {
	...
	if val == 401 {
		sender.Reply("您的CK已失效或者格式错误，请重新提交")
		return false
	} else {
		date := Date()
...
func Meituan_Bind(sender *Sender, prefix string, uuid string) bool {
	...
	if len(cks) == 0 {
		...
		sender.Reply("匹配到多个美团账号,进入手动uuid匹配模式")
		return false

	}
	for _, ck := range cks {
		ck.Updates(MeiTuan{UUID: uuid})
		sender.Reply(fmt.Sprintf("uuid绑定成功:%s", ck.Nickname))
	}
	return true
}
...
func (ck *MeiTuan) RunTT(sender *Sender) {
	...
	logs.Info(replexQuan(string(output), sender))
...
func replexQuan(info string, sender *Sender) string {
	...
	msgs := []string{
		fmt.Sprintf("订单类型:美团领券，订单编号：%s，领卷完成，共计领卷%d张,明细如下:", GetEnv("orderNumber2"), len(matches)),
	}
	for _, match := range matches {
		...
		case 1:
			...
			if reg.MatchString(n) {
				...
				if len(meiTuans) <= num {
					...
					orderNum := GetEnv("orderNumber2") // 获取case 2 的当前
					if orderNum == "" {
						env := &Env{
							Name:  "orderNumber2",
							Value: "1",
					...
					if result2 {
						if sender.IsAdmin {
							...
							if value == "" {
								...
								if coin < jbcoin {
									sender.Reply(fmt.Sprintf("积分不足，美团领券功能需要%d个积分，请直接私聊微信机器人转账，1元=100积分，转账成功即可完成积分充值，或者复制网址http://180.152.5.230:8005/到其他浏览器打开购买卡密充值）", jbcoin))
									meituanList[sender.UserID] = nil
									return
								}
								RemCoin(sender.UserID, jbcoin)
								sender.Reply(fmt.Sprintf("开始美团领券，已扣除%d个积分，剩余积分%d，：订单编号：%s", jbcoin, GetCoin(sender.UserID), GetEnv("orderNumber2")))
							}
						}
					} else {
						sender.Reply("选择序列号账号CK失效，请重新发送[美团登录]提交ck")
						meituanList[sender.UserID] = nil
						return
					}
			...
			orderNum, _ := strconv.Atoi(GetEnv("orderNumber2"))
			orderNum++ // case 2 的订单编号自增+1
			env1 := &Env{
				Name:  "orderNumber2",
				Value: strconv.Itoa(orderNum),
			}
			ExportEnv(env1)
...
func Meituan_getck(sender *Sender) {
	...
	sender.SendImg(png)
	sender.Reply("1、请微信识别或扫描二维码，登录之后点击微信右上角的 ... 点击下面投诉旁边的复制链接发送给机器人")
	return
}
...
func MeituanUuid(sender *Sender, msg chan string, typ string, meituans []MeiTuan) {
	for {
		n, ok := <-msg
		//说明发送方关闭了channel
		if !ok {
		...
		if reg.MatchString(n) {
			...
			if len(meiTuans) <= num {
				...
				meiTuanToUpdate := meiTuans[num]
				ck2 := meituans[num].Token
				result2 := UpLine2(ck2, sender)
				if result2 {
					meiTuanToUpdate.UUID = typ
					db.Model(meiTuanToUpdate).Update("UUID", meiTuanToUpdate.UUID)
					sender.Reply(fmt.Sprintf("更新账号%s的uuid成功！", meituans[num].Nickname))
					meituanList[sender.UserID] = nil
					return
				} else {
					sender.Reply("选择序列号账号CK失效，已退出，请先更新账号！")
					meituanList[sender.UserID] = nil
					return
				}
			}
