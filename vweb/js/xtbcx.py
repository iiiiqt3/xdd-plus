        ...
        while try_count > 0:
            try:
                ...
                if 'username' in r.text:
                    ...
                    self.zw = self.sc_zw()
                    self.startmsg += f'🆔{self.name}({self.usid})'
                    return True
                elif '失败' in r.text:
                    return False
                else:
    ...
    def Getmtgsig_data(self, urls, headers, body):
        ...
        while try_count > 0:
            try:
                ...
                r = requests.post('http://180.167.44.58:1234/mtgsig', json=data)
        ...
        while try_count > 0:
            try:
                ...
                if rj['msg'] == 'ok':
                    ...
                    for d in data:
                        self.xtb = d['count']
                        self.startmsg += f'🎁当前小团币: {int(self.xtb)}({int(self.xtb) / 1000}元)\n'
                    return True
                else:
                    if isprint:
                        self.startmsg += f'🆔账号{self.num}-{self.name} 查询当前团币失败：还有{try_count - 1}次重试机会\n'
                    try_count -= 1
                    time.sleep(random.randint(3, 8))
                    continue
            except:
                if isprint:
                    self.startmsg += f'🆔账号{self.num}-{self.name} 查询当前团币异常：还有{try_count - 1}次重试机会\n'
                try_count -= 1
                time.sleep(random.randint(3, 8))
                continue
